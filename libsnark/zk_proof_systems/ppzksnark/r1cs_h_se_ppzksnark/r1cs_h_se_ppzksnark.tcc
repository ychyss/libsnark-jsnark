...
#ifndef R1CS_H_SE_PPZKSNARK_TCC_
#define R1CS_H_SE_PPZKSNARK_TCC_
...
// 用来hash序列化点
typedef libff::bigint<4L> bigint256;
typedef libff::bigint<8L> bigint512;

/**
 * 将坐标连接在一起,形成了一个超长的字符串
*/
std::string array2String(const std::vector<std::string>& strings) {
    std::string result;
...
/**
 * 先对字符串hash256,然后输出为bigint
*/
bigint256 sha256ToInt(const std::string& strhash) {
    ...
    mpz_t mpz_hash;
    mpz_init(mpz_hash);
    mpz_import(mpz_hash, SHA256_DIGEST_LENGTH, 1, sizeof(hash[0]), 0, 0, hash);
    result = bigint256(mpz_hash);
    mpz_clear(mpz_hash);
...
/**
 * 先对字符串hash512,然后输出为bigint
*/
bigint512 sha512ToInt(const std::string& input) {
    ...
    mpz_t mpz_hash;
    mpz_init(mpz_hash);
    mpz_import(mpz_hash, SHA512_DIGEST_LENGTH, 1, sizeof(hash[0]), 0, 0, hash);
    result = bigint512(mpz_hash);
    mpz_clear(mpz_hash);
...
template<typename ppT>
bool r1cs_h_se_ppzksnark_proving_key<ppT>::operator==(const r1cs_h_se_ppzksnark_proving_key<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_h_se_ppzksnark_proving_key<ppT> &pk)
{
...
template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_h_se_ppzksnark_proving_key<ppT> &pk)
{
    in >> pk.alpha_g1;
...
template<typename ppT>
bool r1cs_h_se_ppzksnark_verification_key<ppT>::operator==(const r1cs_h_se_ppzksnark_verification_key<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_h_se_ppzksnark_verification_key<ppT> &vk)
{
    ...
    out << vk.delta_g1 << OUTPUT_NEWLINE; // 输出

    return out;
}

template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_h_se_ppzksnark_verification_key<ppT> &vk)
{
    ...
    libff::consume_OUTPUT_NEWLINE(in);
    in >> vk.delta_g1;
    libff::consume_OUTPUT_NEWLINE(in); // 读取

    return in;
}

template<typename ppT>
bool r1cs_h_se_ppzksnark_processed_verification_key<ppT>::operator==(const r1cs_h_se_ppzksnark_processed_verification_key<ppT> &other) const
{
    return (this->vk_alpha_g1_beta_g2 == other.vk_alpha_g1_beta_g2 &&
            ...
            this->vk_delta_g1 == other.vk_delta_g1 &&
            this->vk_delta_g2 == other.vk_delta_g2 &&
            this->gamma_ABC_g1 == other.gamma_ABC_g1);
}

template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_h_se_ppzksnark_processed_verification_key<ppT> &pvk)
{
    ...
    out << pvk.vk_delta_g1 << OUTPUT_NEWLINE; // 输出
    out << pvk.vk_delta_g2 << OUTPUT_NEWLINE; // 输出

    return out;
}

template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_h_se_ppzksnark_processed_verification_key<ppT> &pvk)
{
...
template<typename ppT>
bool r1cs_h_se_ppzksnark_proof<ppT>::operator==(const r1cs_h_se_ppzksnark_proof<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_h_se_ppzksnark_proof<ppT> &proof)
{
...
template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_h_se_ppzksnark_proof<ppT> &proof)
{
    in >> proof.g_A;
    libff::consume_OUTPUT_NEWLINE(in);
    in >> proof.g_B;
...
template<typename ppT>
r1cs_h_se_ppzksnark_verification_key<ppT> r1cs_h_se_ppzksnark_verification_key<ppT>::dummy_verification_key(const size_t input_size)
{
    r1cs_h_se_ppzksnark_verification_key<ppT> result;
...
template <typename ppT>
r1cs_h_se_ppzksnark_keypair<ppT> r1cs_h_se_ppzksnark_generator(const r1cs_h_se_ppzksnark_constraint_system<ppT> &r1cs)
{
    libff::enter_block("Call to r1cs_h_se_ppzksnark_generator");

    /* Make the B_query "lighter" if possible */
    r1cs_h_se_ppzksnark_constraint_system<ppT> r1cs_copy(r1cs);
    r1cs_copy.swap_AB_if_beneficial();

    /* Generate secret randomness */
    // 找了一些群上的随机元素
    const libff::Fr<ppT> t = libff::Fr<ppT>::random_element(); // trusted setup secret
    ...
    const libff::Fr<ppT> gamma_inverse = gamma.inverse(); // 好用的变量
    ...
    libff::enter_block("Compute query densities"); // 这里获取了电路中所有非零的元素
    ...
    libff::leave_block("Call to r1cs_h_se_ppzksnark_generator");

    accumulation_vector<libff::G1<ppT> > gamma_ABC_g1(std::move(gamma_ABC_g1_0), std::move(gamma_ABC_g1_values));

    r1cs_h_se_ppzksnark_verification_key<ppT> vk = r1cs_h_se_ppzksnark_verification_key<ppT>(alpha_g1_beta_g2,
                                                                                         gamma_g2,
                                                                                         delta_g2,
                                                                                         delta_g1,
                                                                                         gamma_ABC_g1);

    r1cs_h_se_ppzksnark_proving_key<ppT> pk = r1cs_h_se_ppzksnark_proving_key<ppT>(std::move(alpha_g1),
                                                                               std::move(beta_g1),
                                                                               std::move(beta_g2),
                                                                               std::move(delta_g1),
                                                                               std::move(delta_g2),
    ...
    pk.print_size();
    vk.print_size();
    // 各个参数分别封装成了pk和k，其实还是那一堆参数
    return r1cs_h_se_ppzksnark_keypair<ppT>(std::move(pk), std::move(vk));
}

template <typename ppT>
r1cs_h_se_ppzksnark_proof<ppT> r1cs_h_se_ppzksnark_prover(const r1cs_h_se_ppzksnark_proving_key<ppT> &pk,
                                                      const r1cs_h_se_ppzksnark_primary_input<ppT> &primary_input,
                                                      const r1cs_h_se_ppzksnark_auxiliary_input<ppT> &auxiliary_input)
{
    libff::enter_block("Call to r1cs_h_se_ppzksnark_prover");
    ...
    // libff::bigint<4> hashAInt = ;
    
    // libff::Fr<ppT> hashA = libff::Fr<ppT>(); // 这里需要找到bigint的位数
    ...
    printf("证明修改:\n");
    // libff::Fr<ppT> A_Hash = libff::Fr<ppT>(sha512ToInt(g1Point2String<ppT>(g1_A)) % libff::Fr<ppT>::mod);
    // printf("mod:\n");
    // libff::Fr<ppT>::mod.print();
    bigint512 mod = bigint512(libff::Fr<ppT>::mod.to_string().c_str());
    // printf("mod str:\n");
    // mod.print();

    bigint512 g_A_Hash = sha512ToInt(g1Point2String<ppT>(g1_A));
    bigint512 mod_A_Hash_512 = g_A_Hash % mod;
    // printf("mod_A_Hash_512:\n");
    // mod_A_Hash_512.print();
    bigint256 mod_A_Hash_256 = bigint256(mod_A_Hash_512.to_string().c_str());
    // printf("mod_A_Hash_256:\n");
    // mod_A_Hash_256.print();
    libff::Fr<ppT> A_Hash = libff::Fr<ppT>(mod_A_Hash_256);
    
    // libff::Fr<ppT> B_Hash = libff::Fr<ppT>(sha512ToInt(g2Point2String<ppT>(g2_B)) % libff::Fr<ppT>::mod);
    bigint512 g_B_Hash = sha512ToInt(g2Point2String<ppT>(g2_B));
    bigint512 mod_B_Hash_512 = g_B_Hash % mod;
    // printf("mod_B_Hash_512:\n");
    // mod_B_Hash_512.print();
    bigint256 mod_B_Hash_256 = bigint256(mod_B_Hash_512.to_string().c_str());
    // printf("mod_B_Hash_256:\n");
    // mod_B_Hash_256.print();
    libff::Fr<ppT> B_Hash = libff::Fr<ppT>(mod_B_Hash_256);


    libff::G1<ppT> temp_ret_A = A_Hash * g1_B; // 这里的乘法，有顺序
    // printf("temp ret A=\n");
    // temp_ret_A.print_coordinates();
    libff::G1<ppT> temp_ret_B = B_Hash * g1_A;
    // printf("temp ret B=\n");
    // temp_ret_B.print_coordinates();
    libff::G1<ppT> temp_ret_delta = (A_Hash * B_Hash) * pk.delta_g1;
    // printf("temp ret delta=\n");
    // temp_ret_delta.print_coordinates();
    ...
    printf("old g_C=\n");
    g1_C.print_coordinates();
    /* + H(a) * b + H(b) * a + delta * (H(a)+ H(b)) */
    g1_C =  g1_C + temp_ret_A + temp_ret_B + temp_ret_delta;
    printf("new g_C=\n");
    g1_C.print_coordinates();

    libff::leave_block("Compute the proof");

    libff::leave_block("Call to r1cs_h_se_ppzksnark_prover");

    r1cs_h_se_ppzksnark_proof<ppT> proof = r1cs_h_se_ppzksnark_proof<ppT>(std::move(g1_A), std::move(g2_B), std::move(g1_C));
    proof.print_size();
...
template <typename ppT>
r1cs_h_se_ppzksnark_processed_verification_key<ppT> r1cs_h_se_ppzksnark_verifier_process_vk(const r1cs_h_se_ppzksnark_verification_key<ppT> &vk)
{
    libff::enter_block("Call to r1cs_h_se_ppzksnark_verifier_process_vk");

    r1cs_h_se_ppzksnark_processed_verification_key<ppT> pvk;
    ...
    pvk.vk_delta_g1 = vk.delta_g1; // 新增
    pvk.vk_delta_g2 = vk.delta_g2; // 新增

    pvk.gamma_ABC_g1 = vk.gamma_ABC_g1;

    libff::leave_block("Call to r1cs_h_se_ppzksnark_verifier_process_vk");

    return pvk;
}

template <typename ppT>
bool r1cs_h_se_ppzksnark_online_verifier_weak_IC(const r1cs_h_se_ppzksnark_processed_verification_key<ppT> &pvk,
                                               const r1cs_h_se_ppzksnark_primary_input<ppT> &primary_input,
                                               const r1cs_h_se_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_h_se_ppzksnark_online_verifier_weak_IC");
    ...
    bigint512 g_A_Hash = sha512ToInt(g1Point2String<ppT>(proof.g_A));
    bigint512 mod_A_Hash_512 = g_A_Hash % mod;
    // printf("mod_A_Hash_512:\n");
    // mod_A_Hash_512.print();
    bigint256 mod_A_Hash_256 = bigint256(mod_A_Hash_512.to_string().c_str());
    ...
    // mod_A_Hash_256.print();
    libff::Fr<ppT> A_Hash = libff::Fr<ppT>(mod_A_Hash_256);
    // libff::Fr<ppT> A_Hash = libff::Fr<ppT>(sha512ToInt(g1Point2String<ppT>(proof.g_A)) % libff::Fr<ppT>::mod);

    bigint512 g_B_Hash = sha512ToInt(g2Point2String<ppT>(proof.g_B));
    bigint512 mod_B_Hash_512 = g_B_Hash % mod;
    // printf("mod_B_Hash_512:\n");
    // mod_B_Hash_512.print();
    bigint256 mod_B_Hash_256 = bigint256(mod_B_Hash_512.to_string().c_str());
    ...
    // mod_B_Hash_256.print();
    libff::Fr<ppT> B_Hash = libff::Fr<ppT>(mod_B_Hash_256);
    // libff::Fr<ppT> B_Hash = libff::Fr<ppT>(sha512ToInt(g2Point2String<ppT>(proof.g_B)) % libff::Fr<ppT>::mod);
    ...
    printf("g_A:\n");
    proof.g_A.print_coordinates();
    printf("g_B:\n");
    proof.g_B.print_coordinates();
    printf("g_C:\n");
    proof.g_C.print_coordinates();

    // 这里给G1_precomp和G2_precomp添加了乘法运算
    printf("correct g_A:\n");
    const libff::G1<ppT> correct_g_A = proof.g_A + A_Hash * pvk.vk_delta_g1;
    correct_g_A.print_coordinates();
    printf("correct g_B:\n");
    const libff::G2<ppT> correct_g_B = proof.g_B + B_Hash * pvk.vk_delta_g2;
    correct_g_B.print_coordinates();

    // 这里计算之后还是点，但是坐标不同
    const libff::G1_precomp<ppT> proof_g_A_precomp = ppT::precompute_G1(correct_g_A);
    const libff::G2_precomp<ppT> proof_g_B_precomp = ppT::precompute_G2(correct_g_B);
    ...
    // printf("g_A(Jacobian):\n");
    // proof_g_A_precomp.print_coordinates();
    // printf("g_B(Jacobian):\n");
    // proof_g_B_precomp.print_coordinates();
    // printf("g_C(Jacobian):\n");
    // proof_g_C_precomp.print_coordinates();
    ...
    libff::leave_block("Call to r1cs_h_se_ppzksnark_online_verifier_weak_IC");

    return result;
}

template<typename ppT>
bool r1cs_h_se_ppzksnark_verifier_weak_IC(const r1cs_h_se_ppzksnark_verification_key<ppT> &vk,
                                        const r1cs_h_se_ppzksnark_primary_input<ppT> &primary_input,
                                        const r1cs_h_se_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_h_se_ppzksnark_verifier_weak_IC");
    r1cs_h_se_ppzksnark_processed_verification_key<ppT> pvk = r1cs_h_se_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_h_se_ppzksnark_online_verifier_weak_IC<ppT>(pvk, primary_input, proof);
    libff::leave_block("Call to r1cs_h_se_ppzksnark_verifier_weak_IC");
    return result;
}

template<typename ppT>
bool r1cs_h_se_ppzksnark_online_verifier_strong_IC(const r1cs_h_se_ppzksnark_processed_verification_key<ppT> &pvk,
                                                 const r1cs_h_se_ppzksnark_primary_input<ppT> &primary_input,
                                                 const r1cs_h_se_ppzksnark_proof<ppT> &proof)
{
    bool result = true;
    libff::enter_block("Call to r1cs_h_se_ppzksnark_online_verifier_strong_IC");
    ...
    {
        result = r1cs_h_se_ppzksnark_online_verifier_weak_IC(pvk, primary_input, proof);
    }

    libff::leave_block("Call to r1cs_h_se_ppzksnark_online_verifier_strong_IC");
    return result;
}

template<typename ppT>
bool r1cs_h_se_ppzksnark_verifier_strong_IC(const r1cs_h_se_ppzksnark_verification_key<ppT> &vk,
                                          const r1cs_h_se_ppzksnark_primary_input<ppT> &primary_input,
                                          const r1cs_h_se_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_h_se_ppzksnark_verifier_strong_IC");
    r1cs_h_se_ppzksnark_processed_verification_key<ppT> pvk = r1cs_h_se_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_h_se_ppzksnark_online_verifier_strong_IC<ppT>(pvk, primary_input, proof);
    libff::leave_block("Call to r1cs_h_se_ppzksnark_verifier_strong_IC");
    return result;
}

template<typename ppT>
bool r1cs_h_se_ppzksnark_affine_verifier_weak_IC(const r1cs_h_se_ppzksnark_verification_key<ppT> &vk,
                                               const r1cs_h_se_ppzksnark_primary_input<ppT> &primary_input,
                                               const r1cs_h_se_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_h_se_ppzksnark_affine_verifier_weak_IC");
    ...
    libff::leave_block("Call to r1cs_h_se_ppzksnark_affine_verifier_weak_IC");

    return result;
}

} // libsnark
#endif // R1CS_H_SE_PPZKSNARK_TCC_
